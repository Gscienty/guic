...
{
    struct rbroot *root = (struct rbroot *) malloc(sizeof(struct rbroot));
...
{
    struct rbnode *ret = (struct rbnode *) malloc(size);

    // init
    ret->color  = RBTREE_RED;
    ret->left   = (struct rbnode *) &root->nil;
    ret->right  = (struct rbnode *) &root->nil;
    ret->parent = (struct rbnode *) &root->nil;
